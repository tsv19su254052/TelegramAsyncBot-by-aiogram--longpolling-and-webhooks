...
async def settings(cq: types.CallbackQuery):
    await cq.answer()
    await cq.message.edit_text(_('Settings'), reply_markup=bot.keyboards.settings())


@dp.callback_query_handler(settings_cd.filter(set='lang'))
async def show_lang(cq: types.CallbackQuery, locale: str):
    await cq.answer()
    await cq.message.edit_text(_('Choose language'), reply_markup=bot.keyboards.lang(locale))
...
async def show_admin_panel(cq: types.CallbackQuery):
    ...
    await cq.message.edit_text(hbold(_('Admin panel')) + _('\nUsers count: {uc}').format(uc=users_count), reply_markup=bot.keyboards.admin())


@dp.callback_query_handler(lang_cd.filter())
async def choose_lang(cq: types.CallbackQuery, callback_data: dict, user: User):
    await cq.answer()
    ...
    await cq.message.edit_text(_("Settings", locale=lang), reply_markup=bot.keyboards.settings(locale=lang))


@dp.callback_query_handler(page_cd.filter())
async def pagination(cq: types.CallbackQuery, callback_data: dict):
    await cq.answer()
    ...
    await cq.message.edit_text(f"Inline Pagination | {page}", reply_markup=bot.keyboards.page(page, last, words))


@dp.callback_query_handler(word_cd.filter())
async def word(cq: types.CallbackQuery, callback_data: dict):
